...
class CycleGAN:
  def __init__(self,
    ...
    self.g_mask_x = tf.placeholder(tf.float32,
        shape=[batch_size, image_size, image_size, 3])
    self.g_mask_y = tf.placeholder(tf.float32,
        shape=[batch_size, image_size, image_size, 3])

    self.g_mask_fake_y = tf.placeholder(tf.float32,
        shape=[batch_size, image_size, image_size, 3])
    self.g_mask_fake_x = tf.placeholder(tf.float32,
        shape=[batch_size, image_size, image_size, 3])
    self.g_lambda = g_lambda

    # discriminator hint staff yw3025
    # define mask variable
    self.mask_x = tf.placeholder(tf.float32, shape=[batch_size, image_size, image_size, 3])
    self.mask_y = tf.placeholder(tf.float32, shape=[batch_size, image_size, image_size, 3])
  ...
  def model(self):
    ...
    # discriminator hint staff yw3025
    # call loss function
    D_Y_loss = self.discriminator_loss(self.D_Y, y, self.fake_y, use_lsgan=self.use_lsgan, dy=True)
    ...
    # discriminator hint staff yw3025
    # call loss function
    D_X_loss = self.discriminator_loss(self.D_X, x, self.fake_x, use_lsgan=self.use_lsgan,  dy=False)
  ...
  # generator hint staff yw3025
  # add persevation loss
  def railway_loss(self, x, fake_y, gy = True):
    if gy: # generator g
        return tf.reduce_mean(tf.abs(self.g_mask_x * x - self.g_mask_fake_y * fake_y))
    else: # now x is y, fake_y is fake_x
        return tf.reduce_mean(tf.abs(self.g_mask_y * x - self.g_mask_fake_x * fake_y))
